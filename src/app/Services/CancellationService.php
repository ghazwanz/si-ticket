...
final class CancellationService
{
    ...
    public function cancelEvent(Event $event): void
    {
        ...
        if ($event->ticketCategories->sum('sold_count') > 0) {
            throw new \RuntimeException('Acara tidak dapat dibatalkan secara langsung karena sudah ada tiket yang terjual.');
        }
    ...
    public function requestCancellation(Event $event, string $reason, User $organizer): CancellationRequest
    {
        ...
        if ($event->ticketCategories->sum('sold_count') == 0) {
            throw new \RuntimeException('Gunakan pembatalan langsung karena belum ada tiket terjual.');
        }
