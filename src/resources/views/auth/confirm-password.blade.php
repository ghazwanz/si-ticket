<x-guest-layout>
    <div class="text-center">
        ...
        <h1 class="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Secure Area</h1>
        <p class="mt-2 text-sm text-slate-500">Please enter your password to continue.</p>
    </div>
    ...
    <form method="POST" action="{{ route('password.confirm') }}" class="mt-8 space-y-5">
        ...
        <button type="submit" class="group relative flex w-full justify-center mt-6 rounded-xl bg-violet-600 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 transition-all disabled:opacity-50">
            Confirm Password
            <x-heroicon-s-shield-check class="ml-2 h-5 w-5 transition-transform group-hover:scale-110" />
        </button>
    </form>
</x-guest-layout>
