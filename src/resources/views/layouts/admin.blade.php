...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" 
      ...
      x-init="
        ...
        $watch('sidebarMini', val => localStorage.setItem('sidebarMini', val));
        if (window.innerWidth > 768 && window.innerWidth < 1024 && localStorage.getItem('sidebarMini') === null) sidebarMini = true;
      "
      :class="{ 'dark': darkMode }"
...
<head>
    ...
    <title>{{ $title ?? 'JoinFest Admin' }}</title>
...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans antialiased selection:bg-violet-500/30">
    ...
    <aside 
        ...
        <div class="h-16 flex items-center px-6 border-b border-slate-100 dark:border-slate-800 overflow-hidden">
            <div class="flex items-center gap-3 shrink-0">
                <div class="w-8 h-8 rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-600 flex items-center justify-center shadow-lg shadow-violet-600/20">
    ...
    <div :class="[
        ...
        {{-- Header / Topbar --}}
        <header 
            :class="scrolled ? 'glass-panel shadow-sm py-2' : 'bg-transparent py-4'"
            class="sticky top-0 z-40 px-6 flex items-center justify-between transition-all duration-300">
            
            <div class="flex items-center gap-4">
                <button @click="if(window.innerWidth > 1024) { sidebarMini = !sidebarMini } else { sidebarOpen = !sidebarOpen }" class="p-2 rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
            ...
            <div class="flex items-center gap-3">
                ...
                {{-- Dark Mode Toggle --}}
                <button @click="darkMode = !darkMode" class="p-2 rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                ...
                <div class="relative ml-2" x-data="{ open: false }" @click.away="open = false">
                    <button @click="open = !open" class="flex items-center gap-3 p-1 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                        ...
                        <div class="hidden sm:block text-left">
                            ...
                            <div class="text-[10px] text-slate-400 font-medium mt-0.5 uppercase tracking-tighter">Administrator</div>
                        </div>
                    ...
                    <div 
                        ...
                        class="absolute right-0 mt-3 w-48 glass-panel rounded-2xl shadow-xl py-2 z-50">
                        ...
                        <a href="{{ route('admin.settings.index') }}" data-link class="flex items-center gap-3 px-4 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-violet-500 hover:text-white transition-colors">
        ...
        <footer class="px-6 py-4 border-t border-slate-100 dark:border-slate-800 text-center">
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">&copy; {{ date('Y') }} Ekosistem JoinFest. Dibangun untuk performa.</p>
        </footer>
    </div>
